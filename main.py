...
# AI Pipe Client (Correct Address)
client = OpenAI(
...
def home():
    return {
        ...
        "capabilities": ["Web Scraping", "PDF/CSV Reading", "Turbo Mode", "Background Processing"]
    }
...
# --- 4. MAIN LOGIC (BACKGROUND PROCESSING) ---
async def process_quiz_loop(start_url: str, email: str, secret: str):
    ...
    while current_url and current_url not in visited_urls:
        ...
        try:
            ...
            # B. ASK AI PIPE (STRICT INSTRUCTIONS)
            prompt = f"""
...
# --- 5. API ENDPOINT (BACKGROUND TASK IS CRITICAL) ---
@app.post("/quiz")
async def receive_task(task: QuizTask, background_tasks: BackgroundTasks):
    ...
    # Ye line sabse important hai: BackgroundTask
